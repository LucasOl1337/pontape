...
Atualizado: 24/09/2026, tarde · Site sem crédito pessoal (D036), contador de visitas (D037), **sem Bitcoin** (D038), transparência com corrente animada e selo novo (F42 E1, PRs #100 a #103), **Lastro (Grok 4.7) no E3**, carimbo público real no ar (D040), transparência em uma tela (#104), **Yume no ar (F41, D039)**, F43 no ar (contador, admin, histórico), deploy consertado (#118) · **Divulgação começou** (D035): `/perguntas` com quem faz e contato (PR #97), posts no X e no LinkedIn do Lucas publicados pelo Regente · Home em tom de marca (D034, PR #96) e lema caridade inteligente (D033) · **Lema novo: caridade inteligente** (D033, PR #91) · **Site no ar em https://pontape.org** (D019), publicação automática ligada (D021)
...
|---|---|---|---|---|---|
| F43 | [Contador público, painel do admin e histórico da Yume](frentes/F43-metricas-admin/BRIEF.md) | tecla | `tecla/f43-metricas` | **No ar** (#117): contador no pé da escada, `/admin` com usuário e senha, conversas da Yume guardadas; migração aplicada no D1 | Acompanhar números; Tecla em espera |
...
| F42 | [Transparência que prova e recebe](frentes/F42-transparencia-que-recebe/BRIEF.md) | lastro | `lastro/f42-e2` | E1, E1b, E2 e **E2b integradas** (#100, #102, #108, #110): escrevente no Actions, carimbo diário no Rekor e freetsa, **selo verde no ar** (D040); corrente em cinco cenas sem Bitcoin (#103) e uma tela (#104); Regente destravou o selo e o contador (#111) | **E3 integrada** (#113): /doar fechada por flag (noindex, fora do menu), checkout Asaas, webhook com token, evento finance sem pessoa, reconciliação diária, carimbo que falha não trava o livro. Tudo desligado no ar | Espera a conta sandbox do Asaas (Lucas). Depois: secrets `ASAAS_SANDBOX_KEY`, `ASAAS_WEBHOOK_TOKEN`, `LEDGER_DISPATCH_TOKEN` e uma doação de teste de ponta a ponta. E4 (dinheiro real) só com associação e OK do Lucas |
...
| ID | Frente | Resultado |
|---|---|---|
| F36 | Escada: meio termo entre o antes e o agora | [PR #86](https://github.com/LucasOl1337/pontape/pull/86) 13:37 e [#89](https://github.com/LucasOl1337/pontape/pull/89) 13:44 integradas (D031). A escada de antes de volta, com o nome de cada passo e a linha da IA embaixo a partir do Sistema, ar de 40 a 71 px embaixo em toda janela de computador; número a 8 px do chão e longe do ícone; nomes no tablet; no celular, só os números. Causa: a F33 levou o ar do botão Cores, e o Brave do Lucas escala o texto em 1,18 |
