...
3. Confira a CSP do `_headers` no HTML final. A página atual tem CSS embutido e nenhum script embutido; a regra permite CSS embutido, scripts da própria origem e o hash do script **exato** do protótipo. O protótipo também usa Google Fonts, permitidas na CSP. Se a F07 alterar o script ou Astro gerar outro script embutido, externalize-o ou recalcule o hash e teste no navegador antes do deploy. `microphone=()` fica assim até uma frente de entrevista autorizada. [Cloudflare, formato de `_headers`, acesso em 22/09/2026](https://developers.cloudflare.com/pages/configuration/headers/).
