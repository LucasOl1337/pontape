...
3. O `npm run build` roda Astro, calcula SHA-256 de **todos os scripts embutidos em `dist/**/*.html`**, acrescenta os hashes à CSP de `dist/_headers` e verifica a cobertura. O build falha se faltar hash ou se `script-src` tiver `unsafe-inline`. `public/_headers` é só o modelo, sem hash manual: confira **`dist/_headers`**, que é o arquivo publicado. Desde a F07 (PR #32) o site é gerado pelo Astro e as fontes são servidas do próprio site: a CSP não permite Google Fonts nem outra origem externa. CSS embutido segue permitido. Os dados do livro vão em `<script type="application/json">`, que não executa; o build também põe o hash deles. Teste também no navegador, pois a verificação automática não detecta toda falha de carregamento ou interação. `microphone=()` fica assim até uma frente de entrevista autorizada. [Cloudflare, formato de `_headers`, acesso em 22/09/2026](https://developers.cloudflare.com/pages/configuration/headers/).

4. Defina a variável de ambiente `SITE_URL` com o endereço final (ex.: `https://DOMINIO_ESCOLHIDO`) no build do Pages. Sem ela, as metatags de prévia (Open Graph) saem sem endereço absoluto e WhatsApp e X não acham a imagem do link (F16, PR #33).
...
**Voltar atrás hoje (Workers):** siga [`VOLTAR-ATRAS.md`](VOLTAR-ATRAS.md): parar o timer, `wrangler rollback`, conferir, revert na `main`, religar o timer. O parágrafo abaixo é do roteiro de Pages e fica como histórico.
