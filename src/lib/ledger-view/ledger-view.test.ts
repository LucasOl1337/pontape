...
describe('últimas ações em palavra de gente (F32)', () => {
  ...
  it('junta as mudanças seguidas do mesmo dia numa linha, a mais nova primeiro', async () => {
    ...
    expect(recentLines(events, plain)).toEqual([
      { day: '2000-01-01', through: '5', text: 'Uma mudança aprovada no projeto.' },
      { day: '2000-01-01', through: '4', lead: 'Decisão.', text: 'Frase de D010.' },
      { day: '2000-01-01', through: '3', text: '2 mudanças aprovadas no projeto.' },
      { day: '1999-12-31', through: '1', text: 'Uma mudança aprovada no projeto.' },
    ]);
  });
  ...
  it('para no limite e fala de dinheiro e entregas com as frases do livro', async () => {
    ...
    expect(recentLines(events, plain, 2)).toEqual([
      { day: '2000-01-01', through: '4', text: '2 mudanças aprovadas no projeto.' },
      { day: '2000-01-01', through: '2', text: '3 entregas de comida.' },
    ]);
    expect(recentLines(events, plain).at(-1)).toEqual({ day: '2000-01-01', through: '1', text: 'Doação recebida: R$\u00a050,00.' });
  });
});
